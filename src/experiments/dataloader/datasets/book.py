...
def load_book(data_folder: str = f"data/Hamlet/BookCrossing") -> IlargiMatrix:
    join_set1 = (
            np.genfromtxt(f"{data_folder}/MLFK1.csv", skip_header=True, dtype=int) - 1
    )
    R1 = mmread(
        f"{data_folder}/MLR1Sparse.txt",
    ).tocsr()
    ...
    R2 = mmread(
        f"{data_folder}/MLR2Sparse.txt",
    ).tocsr()

    Y = np.matrix(np.genfromtxt(f"{data_folder}/MLY.csv", skip_header=True)).T
